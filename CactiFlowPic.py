...
def draw_one_pic(one_vlan_list):
    for index_vlan, one_vlan in enumerate(one_vlan_list):
        ...
        pic1_1.set_title('Vlan ' + str(one_vlan) + '  real-time')
        for row_today in today_data.itertuples():
            today_data.at[row_today.Index, 'Date'] = today_data.at[row_today.Index, 'Date'].split(' ')[1][:5]
        pic1_1.plot(today_data['Date'], today_data['Inbound'], label='Inbound')
        pic1_1.plot(today_data['Date'], today_data['Outbound'], label='Outbound')
        ...
        pic1_1.set_xticks([i for i in range(0, len(today_data), 20)])
        pic1_1.set_xlabel('  Inbound:  Current:' + str(today_data.at[len(today_data)-1, 'Inbound']) + '  Average:' + str(today_inbound_ave) + '  Maximum:' + str(today_inbound_max) + '\n'
        ...
        pic1_1.set_ylabel('Gbits  per  second')
        pic1_1.legend(loc='best')

        pic1_2 = plt.subplot2grid((pic_row, pic_col), (pic_row_loc, 11), rowspan=the_rowspan, colspan=the_colspan)  #
        pic1_2.set_title('Vlan ' + str(one_vlan) + '  Compared with yesterday')
        two_days_Inbound = []
        two_days_Outbound = []
        ...
        for row_today in today_data.itertuples():
            for row_yesterday in yesterday_data.itertuples():
                if today_data.at[row_today.Index, 'Date'] == yesterday_data.at[row_yesterday.Index, 'Date']:
                    ...
                    break
        pic1_2.plot(today_data['Date'], two_days_Inbound[:len(today_data)], label='Inbound')
        pic1_2.plot(today_data['Date'], two_days_Outbound[:len(today_data)], label='Outbound')
        pic1_2.set_xticks([i for i in range(0, len(today_data), 20)])
        pic1_2.set_xlabel('Today - yesterday   (The same time)')
        pic1_2.set_ylabel('Gbits')
        pic1_2.legend(loc='best')

        pic1_3 = plt.subplot2grid((pic_row, pic_col), (pic_row_loc, 22), rowspan=the_rowspan, colspan=the_colspan)  #
        pic1_3.set_title('Vlan ' + str(one_vlan) + '  Compared with last week')
        '''
        two_weeks_Inbound = []
        two_weeks_Outbound = []
        ...
        pic1_3.plot(this_week_data['Date'], two_weeks_Inbound[:len(this_week_data)], label='Inbound')
        pic1_3.plot(this_week_data['Date'], two_weeks_Outbound[:len(this_week_data)], label='Outbound')
        pic1_3.set_xticks([i for i in range(0, len(this_week_data), 200)])
        pic1_3.set_xlabel('this week - last week   (The same time)')
        pic1_3.set_ylabel('Gbits')
        pic1_3.legend(loc='best')
        '''
